...
gulp.task('styles', runSequence('clean:styles', 'stylelint'), () => {
  return gulp.src([dir.src + '/_assets/scss/**/*.scss'])
    .pipe(plumber({errorHandler: notify.onError('<%= error.message %>')}))
...
gulp.task('scripts', () => {
  return gulp.src([dir.src + '/_assets/ts/index.ts', dir.src + '/_assets/js/index.js'])
    .pipe(plumber({errorHandler: notify.onError('<%= error.message %>')}))
...
// CSS内のパスを変更する
gulp.task('replaceCssPath', () => {
  return gulp
    .src([dir.dist + '/_assets/css/*'])
    .pipe(replace('/_assets/', '/iroiro/animation/_assets/'))
    .pipe(gulp.dest(dir.dist + '/_assets/css/'));
});
// JS内のパスを変更する
gulp.task('replaceJsPath', () => {
  return gulp
    .src([dir.dist + '/_assets/js/*'])
    .pipe(replace('/_assets/', '/iroiro/animation/_assets/'))
    .pipe(gulp.dest(dir.dist + '/_assets/js/'));
});
// HTML内のパスを変更する
gulp.task('replaceHtmlPath', () => {
  return gulp
    .src([dir.dist + '/**/*.html'])
    .pipe(replace('/_assets/', '/iroiro/animation/_assets/'))
    .pipe(gulp.dest(dir.dist));
...
gulp.task('watch:scripts', () => {
  return watch(
    [
      dir.src + '/_assets/js/**/*.js',
      dir.src + '/_assets/ts/**/*.ts',
      dir.src + '/_assets/ts/**/*.json',
    ], () => {
...
gulp.task('watch', ['watch:styles', 'watch:scripts', 'watch:images', "watch:html", 'watch:pug', 'watch:src']);
...
gulp.task('build', callback => {
  runSequence(
    ['clean:images', 'clean:scripts', 'copy'],
    ['styles', 'scripts', 'imagemin', 'pug'],
    ['replaceCssPath', 'replaceJsPath', 'replaceHtmlPath'],
    // ['htmlmin', 'revision'],
    callback);
});
